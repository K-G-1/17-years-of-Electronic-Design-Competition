...
u8 AT24CXX_Check(void)
{
	...
	if(temp==0X35)return 0;		   
	else//排除第一次初始化的情况
	{
		AT24CXX_WriteOneByte(8191,0X35);
...
u8 IIC_ADD_read(u8 DeviceAddr,u8 address)
{
   ...
   if(EE_TYPE>AT24C16)
	{
		I2c_Soft_SendByte(DeviceAddr);	   //发送写命令
		I2c_Soft_WaitAsk();
		I2c_Soft_SendByte(address>>8);//发送高地址
	 
	}else I2c_Soft_SendByte(0XA0+((address/256)<<1));   //发送器件地址0XA0,写数据 	 
 
    I2c_Soft_SendByte(address%256);   //发送低地址
	I2c_Soft_WaitAsk();	    
	I2c_Soft_Start();  	 	   
...
int16_t IIC_Read_MultiBytes(u8 DeviceAddr,u8 address,u8 Len)
{
	...
	{
		temp<<=8;
		temp+=AT24CXX_ReadOneByte(address+Len-t-1); 	 				   
	}
	return temp;
}
