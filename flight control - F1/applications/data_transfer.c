...
int16_t Openmv_err_x,Openmv_err_y;
int protec_data,a,b,c,d;
char Openmv_err_flag;
float camera_time;
...
extern float Ano_OF_dis_x, Ano_OF_dis_y;
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	{
		f.send_status = 0;
		ANO_DT_Send_Status(Roll,-Pitch,Yaw,OF_ALT2*100,mode_state,fly_ready);	
	}	
/////////////////////////////////////////////////////////////////////////////////////
...
//		ANO_DT_Send_Senser(OF_QUA,OF_DX2,OF_DY2,OF_DX2FIX,OF_DY2FIX,
//							OF_LIGHT,Ano_OF_dis_x,Ano_OF_dis_y,OF_DX2);
//        ANO_DT_Send_Senser(CCD_CTRL_PID.error,CCD_CTRL_PID_H.error,CCD_CTRL_PID.merrer,CCD_CTRL_PID_H.devir,
//                            -Fix_point_except.y,ctrl_2.err.y,ctrl_2.err_i.y,
//							mode_3_duty.flight_time,Ano_OF_dis_x );
...
//							camera_Trance.error_V,camera_Trance.take_off_flag,camera_Trance.trance_flag);
		ANO_DT_Send_Senser(OF_DX2FIX,OF_DY2FIX,Openmv_err_x,Openmv_err_y,
                            OF_ALT2,CCD_CTRL_PID.merrer,CCD_CTRL_PID_H.merrer,
							mode_3_duty.flight_time,Ano_OF_dis_x );
...
//		ANO_DT_Send_Senser2(ultra.relative_height,hight_PID_ctrl.shell.set_hight);//原始数据
		ANO_DT_Send_Senser2(Mode3_Switch.mode,OF_ALT2);//原始数据
	}	
/////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	else if(f.send_pid4)
	{
		...
		ANO_DT_Send_PID(4,pid_setup.groups.ctrl4.kp,pid_setup.groups.ctrl4.ki,pid_setup.groups.ctrl4.kd,
							pid_setup.groups.Forward_time.kp,pid_setup.groups.Forward_time.ki,pid_setup.groups.Forward_time.kd,
							pid_setup.groups.Left_time.kp,pid_setup.groups.Left_time.ki,pid_setup.groups.Left_time.kd				);
	}
    else if(f.send_pid5)
	{
		f.send_pid5 = 0;
		ANO_DT_Send_PID(5,pid_setup.groups.exp_angle.exp_forward,pid_setup.groups.exp_angle.exp_left,pid_setup.groups.exp_angle.exp_back,
						  0,0,0,
                            0,0,0);
	}
...
void Openmv_Data_Receive_Prepare(u8 data)
{
	...
	{
        state =0;
        usart3_save[3+_data_cnt] = data;
//        Openmv_DT_Data_Receive_Anl(RxBuffer);
        memset(usart3_save,0,sizeof(usart3_save));
	}
	else
		state = 0;
}

void Openmv_DT_Data_Receive_Anl(u8 data_buf[32])
{
    sscanf((char *)data_buf,"%*[^:]:%d,%d,%d.",&Openmv_err_x,&Openmv_err_y,&Openmv_err_flag);
    Openmv_err_x = my_deathzoom(Openmv_err_x,0,2);
    Openmv_err_y = my_deathzoom(Openmv_err_y,0,2);
...
void ANO_DT_Data_Receive_Anl(u8 *data_buf,u8 num)
{
	...
	if(*(data_buf+2)==0X13)								//PID4
	{
        ...
        pid_setup.groups.ctrl4.kd  = 0.001*( (vs16)(*(data_buf+8)<<8)|*(data_buf+9) );
         ...
         pid_setup.groups.Forward_time.kp = 0.001*( (vs16)(*(data_buf+10)<<8)|*(data_buf+11) );
         pid_setup.groups.Forward_time.ki = 0.001*( (vs16)(*(data_buf+12)<<8)|*(data_buf+13) );
         pid_setup.groups.Forward_time.kd = 0.001*( (vs16)(*(data_buf+14)<<8)|*(data_buf+15) );
 			
         pid_setup.groups.Left_time.kp 	= 0.001*( (vs16)(*(data_buf+16)<<8)|*(data_buf+17) );
         pid_setup.groups.Left_time.ki 	= 0.001*( (vs16)(*(data_buf+18)<<8)|*(data_buf+19) );
         pid_setup.groups.Left_time.kd 	= 0.001*( (vs16)(*(data_buf+20)<<8)|*(data_buf+21) );
		if(f.send_check == 0)
		{
			f.send_check = 1;
			checkdata_to_send = *(data_buf+2);
	...
	{
        pid_setup.groups.exp_angle.exp_forward  = 0.001*( (vs16)(*(data_buf+4)<<8)|*(data_buf+5) );
        pid_setup.groups.exp_angle.exp_left  = 0.001*( (vs16)(*(data_buf+6)<<8)|*(data_buf+7) );
        pid_setup.groups.exp_angle.exp_back  = 0.001*( (vs16)(*(data_buf+8)<<8)|*(data_buf+9) );
