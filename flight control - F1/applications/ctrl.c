...
xyz_f_t Fix_point_except = {0,0,0};
...
// 	static xyz_f_t acc_no_g_lpf;
    ...
    direction_control(T,Mode3_Switch.mode);                 //方向控制（手动和自动）
//=========================== 期望角度 ========================================
	except_A.x  = MAX_CTRL_ANGLE  *( my_deathzoom( ( CH_filter[ROL]) ,0,30 )/500.0f ) - Fix_point_except.y;   //30
	except_A.y  = MAX_CTRL_ANGLE  *( my_deathzoom( (-CH_filter[PIT]) ,0,30 )/500.0f ) + Fix_point_except.x;  //30
